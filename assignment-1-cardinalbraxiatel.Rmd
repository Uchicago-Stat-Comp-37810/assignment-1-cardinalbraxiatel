...
#4.4.1.2
Again, I am unfamiliar with how to substitute an equation symbollically into R, so I entered the equations and their derivatives by hand.
...
Newton <- function(){
  ...
  i <- 2
  while((x[i]+y[i]) > 0.5 &&  ((x[i])^2 + 2*(y[i])^2 -2) > 0.5){
   #for(i in 2:30) {
  fdx <- 1
  fdy <- 1
  gdx <- 2*xv[i-1]
  gdy <- 4*yv[i-1]
  fn_1 <- xv[i - 1] + yv[i-1]
  gn_1 <- (xv[i-1])^2 +2*(yv[i -1])^2 - 2
  dn_1 = 1*gdy - 1*gdx
  
   xv[i] <- xv[i-1]-(gdy*fn_1 - fdy*gn_1)/dn_1
   yv[i] <- yv[i-1]-(fdx*gn_1 - gdx*fn_1)/dn_1
   i <- i + 1
